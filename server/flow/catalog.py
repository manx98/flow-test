...
_SCRIPT_FUNCS = [
    ...
    _fn("设备.hotkey(keys)", "快捷键：按下并释放组合键，如 ctrl+c / alt+f4（等价「快捷键」）"),
    _fn("设备.hold_keys(keys)", "按住快捷键：作为上下文管理器使用，退出时自动释放（等价「快捷键」hold 分支）"),
...
_NODES = [
    ...
    {"type": "device/rdp", "category": "设备", "title": "RDP/xrdp 设备",
     ...
     "properties": [_pr("host", "string", "127.0.0.1", "RDP 主机地址"),
                    ...
                    _pr("connect_timeout", "number", 60, "RDP 握手超时秒数；调试模式建议 60+"),
                    _pr("first_update_timeout", "number", 30, "等待首帧桌面超时秒数；xrdp/调试模式建议 30+")]},
    ...
    {"type": "action/hotkey", "category": "动作", "title": "快捷键",
     "description": "发送一个快捷键组合。普通模式按下并释放后走 out；连接 hold 分支时，按住组合键执行 hold 分支，结束后释放再走 out。",
     "script": "hotkey(keys)",
     "inputs": [_exec_in(), _p("keys", T.TEXT, "1", desc="快捷键组合（如 ctrl+c、ctrl+shift+t；接入时覆盖属性）"),
                _p("keyboard", T.KEYBOARD, "1", True, desc="目标设备的键盘能力（来自「设备属性」）")],
     "outputs": [_exec_out("hold", "按住分支：按下快捷键后执行，分支结束后自动释放"),
                 _exec_out("out", "释放后继续：普通模式或 hold 分支结束后走这里")],
     "properties": [_pr("keys", "hotkey", "", "快捷键组合；点击迷你键盘选择")]},
