...
func NewRdpClient(host string, width, height int, dialer func(string) (net.Conn, error)) *RdpClient {
	g := &RdpClient{
		...
		width:             width,
		height:            height,
		requestedProtocol: x224.PROTOCOL_SSL | x224.PROTOCOL_HYBRID,
...
// SetRequestedProtocol sets the X224 security protocols requested during login.
// Must be called before Login.
func (g *RdpClient) SetRequestedProtocol(protocol uint32) {
	g.requestedProtocol = protocol
}
...
func (g *RdpClient) doLogin(routingToken []byte) error {
	...
	g.x224.SetRequestedProtocol(g.requestedProtocol)
	if routingToken != nil {
